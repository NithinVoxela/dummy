...
export default function CameraCreate() {
  ...
  const handleSaveCamera = useCallback(async(payload = {}) => {    
    try {   
      if (!isEdit) {
  ...
  return (
    <Page title={`${translate("app.alert-camera-details")} : ${translate("app.alert-new-camera-label")}`}>
      <Container maxWidth={themeStretch ? false : 'lg'}>
        <HeaderBreadcrumbs
          heading={!isEdit ? `${translate("app.camera-add-header-label")}` : `${translate("app.camera-edit-header-label")}`}
          links={[            
        ...
        <CameraNewForm isEdit={isEdit} currentCamera={cameraDetails} translate={translate} handleSave={handleSaveCamera} onCancel={onCancel} />
      </Container>
    </Page>
  );
}
