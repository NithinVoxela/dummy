...
export function NavItemRoot({ item, isCollapse, open = false, active, onOpen, translate }) {
  const { title, path, icon, info, children } = item;

  const renderContent = (
    <>
      {icon && <ListItemIconStyle>{icon}</ListItemIconStyle>}
      <ListItemTextStyle disableTypography primary={translate(`app.${title}-label`)} isCollapse={isCollapse} />
      {!isCollapse && (
        <>
          {info && info}
...
export function NavItemSub({ item, open = false, active = false, onOpen, translate }) {
  const { title, path, info, children } = item;

  const renderContent = (
    <>
      <DotIcon active={active} />
      <ListItemText disableTypography primary={translate(`app.${title}-label`)} />
      {info && info}
      {children && <ArrowIcon open={open} />}
    </>
  );
